#include <bits/stdc++.h>
using namespace std;
const double prob = 0.5;

// Function to generate a random value between 0 and 1
double generateRandomValue() {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<double> dis(0.0, 1.0);
    return dis(gen);
}

//Function to introduce error with probability p
void error(int encoded[], int sz){

    double rand_value = generateRandomValue();
    if(rand_value<=prob){
        int tmp = rand();
        encoded[tmp%(sz)+1] = 1-encoded[tmp%(sz)+1];
    }
}

int check_parity(int n,int i,int code[])
{
    int p=0,k;
    for(int j=i;j<=n;j=k+i)
    {
        for(k=j;k<j+i && k<=n;k++)
        {
            if(code[k]==1)
                p++;
        }
    }
    if(p%2==0)
        return 0;
    else
        return 1;
}

int redundant_bits(int num){
    int r=0;
    while((r+num+1)>(pow(2,r)))
        r++;
    return r;
}

void encode(int code[], int data[], int num){
    int r = redundant_bits(num);
    int m=0,n,j=1,c;
    n=num+r;
    for(int i=1;i<=n;i++)
    {
        if(i==pow(2,m) && m<=r)
        {
            code[i]=0;
            m++;
        }
        else
        {
            code[i]=data[j];
            j++;
        }
    }

    m=0;
    for(int i=1;i<=n;i++)
    {
        if(i==pow(2,m) && m<=r)
        {
            c=check_parity(n,i,code);
            code[i]=c;
            m++;
        }
    }
}

void decode(int received[], int code[], int num){
    int m=0,n,j=0,c=0;
    int r = redundant_bits(num);
    n=num+r;
    for(int i=1;i<=n;i++)
    {
        if(i==pow(2,m) && m<=r)
        {
            c=c+(pow(2,j)*check_parity(n,i,code));
            j++;
            m++;
        }
    }
//    cout<<"c: "<<c<<endl;
    if(code[c]==1)
        code[c]=0;
    else
        code[c]=1;
    m=0,j=1;
    for(int i=1;i<=n;i++)
    {
        if(i==pow(2,m) && m<=r)
        {
            //received[i]=0;
            m++;
        }
        else
        {
            received[j]=code[i];
            j++;
        }
    }
    if(c==0) {
        cout<<"Received Message is correct"<<endl;
    }
    else
    {
        cout<<"There was an error in bit "<<(n-c)+1<<"\nThe corrected Message is:";
        for(int i=n;i>0;i--)
            cout<<code[i];
        cout<<endl;
    }
}





int main()
{
    srand(time(NULL));
    int num;
    cin>>num;
    string s;
    cin>>s;
    int data[num+1];
    for(int i=num;i>0;i--)
        data[i]=s[num-i]-'0';
    int r = redundant_bits(num);
    int n = num+r;
    int code[num+r+1];
    int received[num+1];
    cout<<"Original Message: "<<s<<endl;

    encode(code, data, num);

    cout<<"Encoded Message: ";
    for(int i=n;i>0;i--)
        cout<<code[i];
    cout<<endl;
    error(code, n);
    cout<<"Received Message: ";
    for(int i=n;i>0;i--)
        cout<<code[i];
    cout<<endl;

    decode(received, code, num);

    cout<<"Decoded Message: ";
    for(int i=num;i>0;i--)
        cout<<received[i];
    cout<<endl;


    return 0;
}